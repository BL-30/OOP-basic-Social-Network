...
public class Reflexion {
	...
	public static boolean estUneVariableLocale(String nomFic, Class c, String methode, String var) {
		...
		if (pos>0) {
			String[] primitifs= {"int", "char", "boolean", "float", "long", "double", "String"};
	...
	public static boolean respecteDeveloppementEnCouche(String nomFic, Class c, String methode) {
		...
		for (Field f : fields) {
			if (!Modifier.isStatic(f.getModifiers())) {
				...
				System.out.println(f.getName()+" is static");
			}
		}
		int i=0;
		List<String> mots = getMots(sans);
		while (couche && i<lFields.size()) {
			//System.out.println(fields[i].getName()+" in "+mots);
			if (mots.contains(lFields.get(i).getName())){
				//System.out.println(" mot trouve ="+fields[i].getName()+"<<");
				if (!estUneVariableLocale(nomFic, c, methode,lFields.get(i).getName() )) {
					couche=false;
				}
			}
			i++;
